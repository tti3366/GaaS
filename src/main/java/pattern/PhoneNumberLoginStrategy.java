...
public class PhoneNumberLoginStrategy implements LoginStrategy {
	...
	public ModelAndView login(User user, HttpSession session, HttpServletRequest request, LoginService ls) {
		...
		try {
			...
			User userInfo = ls.selectUserByPhone(user.getUserId(), user.getUserPw());
			
			System.out.println(formatter.format(date) + " [LOGIN] TRY ► " + user.getUserPhoneNumber() + " / " + user.getUserPw());
